...
const Promo = () => {
  const promos = [
    ...
    { icon: 'fa-newspaper', title: 'Insurance Coverage', desc: "Full insurance coverage for your goods throughout the moving process — complete peace of mind." },
        ...
        <div className="mp-promo-grid">
          {promos.map((p, i) => (
            <div key={i} className={`mp-pc sr vis ${i % 3 === 1 ? 'd1' : i % 3 === 2 ? 'd2' : ''}`}>
              <div className="mp-pic"><i className={`fa-solid ${p.icon}`}></i></div>
              <h4>{p.title}</h4>
              <p>{p.desc}</p>
            </div>
          ))}
